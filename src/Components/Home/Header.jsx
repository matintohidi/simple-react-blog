    ...
    return (
        <div className="relative lg:mt-36">
            ...
            <div className="relative block lg:hidden">
                <ImageLazy src={Shape1} className="h-12 w-12 absolute right-10 top-2"/>
                <div className="w-28 h-28 md:w-36 md:h-36 rounded-full absolute top-8 right-7 backdropCircle"></div>
                <ImageLazy src={Shape4} className="md:h-12 md:w-12 h-11 w-11 z-10 absolute right-10 top-12"/>
                <ImageLazy src={Shape3} className="md:h-11 ms:w-11 w-9 h-9 z-10 absolute right-[90px] md:right-[110px] top-16"/>
                <ImageLazy src={Shape2} className="h-10 w-10 z-10 absolute right-14 md:right-20 md:top-28 top-24"/>
                <ImageLazy src={Shape5} className="h-12 w-12 absolute md:right-1/3 right-1/2 top-16 blur-sm"/>
            </div>
            <ImageLazy src={Shape3} className="lg:block hidden absolute right-20 z-20 top-52 h-16 w-16"/>
            <div className="lg:block hidden w-44 h-44 rounded-full absolute backdropCircle z-10 right-10 top-48"></div>
            <ImageLazy src={Shape4} className="lg:block hidden w-16 h-16 absolute z-20 right-24 top-72"/>
            <ImageLazy src={Shape2} className="lg:block hidden h-12 w-12 absolute right-48 top-64"/>
            <ImageLazy src={Shape5} className="lg:block hidden h-20 w-20 absolute -z-10 right-1/4 top-64 blur-sm"/>
            <div className="flex justify-center mt-32 ">
                <div className="animate-bounce flex justify-center items-center rounded-full h-12 w-12 bg-customLightColor hover:bg-[#c9ccd3] transition-colors">
                    <ImageLazy src={ArrorDown} className="w-8 h-8"/>
                </div>
            </div>
        </div>
    )
